...
3. **Gating/advisory tables:** Use `collectIssueMessages` with a spec-specific normaliser so messages dedupe across pages. Pipe the arrays into `renderUnifiedIssuesTable` with `variant: 'gating'` or `'advisory'`. Keep table headings identical to WCAG (Impact, Issue, Viewport(s), Pages, Nodes, WCAG level).
